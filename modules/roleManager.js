...
module.exports = (client) => {
    console.log("[ROLE SYSTEM LOG] Module d'auto-rôle HeLoRiA prêt & sécurisé.");
    ...
    client.on("messageCreate", async (msg) => {
        ...
        if (msg.content === "+setup-roles") {
            console.log(`[ROLE SYSTEM LOG] Commande +setup-roles déclenchée par ${msg.author.tag} (${msg.author.id}) dans #${msg.channel.name}`);
            try {
                if (!msg.member || !msg.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
                    console.warn(`[ROLE SYSTEM LOG] Refus d'exécution de +setup-roles : ${msg.author.tag} n'a pas la permission Administrateur.`);
                ...
                if (channelMessages) {
                    ...
                    console.log(`[ROLE SYSTEM LOG] Nettoyage effectué : ${oldBotMessages.size} ancien(s) message(s) supprimé(s).`);
                }
                ...
                console.log("[ROLE SYSTEM LOG] Panneaux de sélection de rôles déployés avec succès.");

            } catch (error) {
                console.error("[ROLE SYSTEM LOG] Erreur lors de la création du panneau de rôles :", error);
            }
        }
    });
    ...
    client.on("interactionCreate", async (interaction) => {
        ...
        if (interaction.isButton() && interaction.customId.startsWith("cf_role_")) {
            console.log(`[ROLE SYSTEM LOG] Clic sur bouton de confirmation par ${interaction.user.tag} (ID: ${interaction.customId})`);
            try {
                ...
                if (isYes) {
                    ...
                    // Retrait sécurisé des anciens rôles
                    if (categoryConfig) {
                        for (const key in categoryConfig) {
                            ...
                            if (id && member.roles.cache.has(id)) {
                                await member.roles.remove(id).catch(err => 
                                    console.error(`[ROLE SYSTEM LOG] Erreur lors du retrait du rôle ${id} pour ${member.user.tag} :`, err.message)
                                );
                            }
                        }
                    }
                    ...
                    if (selectedValue !== "NON_PRECISE" && targetRoleId && targetRoleId !== "none") {
                        await member.roles.add(targetRoleId).catch(err => 
                            console.error(`[ROLE SYSTEM LOG] Erreur lors de l'ajout du rôle ${targetRoleId} pour ${member.user.tag} :`, err.message)
                        );
                        console.log(`[ROLE SYSTEM LOG] Rôle ${targetRoleId} attribué à ${member.user.tag}.`);
                    }
                    ...
                    console.log(`[ROLE SYSTEM LOG] Modification de rôle annulée par ${member.user.tag}.`);
                ...
                console.error("[ROLE SYSTEM LOG] Erreur lors du clic sur bouton de confirmation :", err);
            }
        }
        ...
        console.log(`[ROLE SYSTEM LOG] Interaction menu "${interaction.customId}" déclenchée par ${interaction.user.tag}`);
        ...
        try {
            ...
            const askConfirmation = async (categoryShort, currentRoleName, targetRoleId) => {
                console.log(`[ROLE SYSTEM LOG] Demande de confirmation envoyée à ${member.user.tag} (Proposé : ${targetRoleId}, Actuel : ${currentRoleName})`);
            ...
            if (interaction.customId === "role_select_genre") {
                if (!config.ROLES_GENRE) {
                    console.error("[ROLE SYSTEM LOG] Option Genre : config.ROLES_GENRE est indéfini.");
                    return await sendResponseEmbed("Configuration Manquante", "La configuration des rôles de genre est introuvable.", false);
                }
                ...
                if (existingRole && existingRole.id !== roleId) {
                    return await askConfirmation("gnr", existingRole.name, roleId);
                }
                ...
                if (selectedValue !== "NON_PRECISE" && roleId) {
                    await member.roles.add(roleId).catch(err => 
                        console.error(`[ROLE SYSTEM LOG] Impossible d'ajouter le rôle genre (${roleId}) à ${member.user.tag} :`, err.message)
                    );
                }

                console.log(`[ROLE SYSTEM LOG] Genre mis à jour pour ${member.user.tag} : ${selectedValue}`);
                return await sendResponseEmbed("Profil Mis à Jour", "Votre genre a été enregistré avec succès.", true);
            }

            // 2. PLATEFORME
            if (interaction.customId === "role_select_plateforme") {
                if (!config.ROLES_PLATEFORME) {
                    console.error("[ROLE SYSTEM LOG] Option Plateforme : config.ROLES_PLATEFORME est indéfini.");
                    return await sendResponseEmbed("Configuration Manquante", "La configuration des plateformes est introuvable.", false);
                }

                const roleId = config.ROLES_PLATEFORME[selectedValue];
                if (!roleId || roleId.startsWith("ID_")) {
                    console.warn(`[ROLE SYSTEM LOG] Rôle plateforme non configuré ou ID fictif détecté : ${selectedValue}`);
                    return await sendResponseEmbed("Configuration Incomplète", "Ce rôle n'est pas encore configuré dans les fichiers du bot.", false);
                }
                ...
                if (existingRole && existingRole.id !== roleId) {
                    return await askConfirmation("plt", existingRole.name, roleId);
                }
                ...
                await member.roles.add(roleId).catch(err => 
                    console.error(`[ROLE SYSTEM LOG] Impossible d'ajouter la plateforme (${roleId}) à ${member.user.tag} :`, err.message)
                );

                console.log(`[ROLE SYSTEM LOG] Plateforme mise à jour pour ${member.user.tag} : ${selectedValue}`);
                return await sendResponseEmbed("Profil Mis à Jour", "Votre plateforme de jeu principale a été mise à jour.", true);
            }

            // 3. NOTIFICATIONS (CHOIX MULTIPLES)
            if (interaction.customId === "role_select_notifs") {
                if (!config.ROLES_NOTIFS) {
                    console.error("[ROLE SYSTEM LOG] Option Notifications : config.ROLES_NOTIFS est indéfini.");
                    return await sendResponseEmbed("Configuration Manquante", "La configuration des notifications est introuvable.", false);
                }
                ...
                console.log(`[ROLE SYSTEM LOG] Notifications mises à jour pour ${member.user.tag} : ${selectedValues.join(", ")}`);
                return await sendResponseEmbed("Préférences Mises à Jour", "Vos abonnements aux notifications ont été ajustés selon vos choix.", true);
            }

            // 4. DIVISION FORTNITE
            if (interaction.customId === "role_select_division") {
                if (!config.ROLES_DIVISION) {
                    console.error("[ROLE SYSTEM LOG] Option Division : config.ROLES_DIVISION est indéfini.");
                    return await sendResponseEmbed("Configuration Manquante", "La configuration des divisions est introuvable.", false);
                }

                const roleId = config.ROLES_DIVISION[selectedValue];

                if (selectedValue === "DIV_1") {
                    console.log(`[ROLE SYSTEM LOG] Demande d'accès Division 1 par ${member.user.tag} (Validation requise).`);
                    return await sendResponseEmbed(
                ...
                if (!roleId || roleId.startsWith("ID_")) {
                    console.warn(`[ROLE SYSTEM LOG] Rôle division non configuré ou ID fictif détecté : ${selectedValue}`);
                    return await sendResponseEmbed("Configuration Incomplète", "Ce rôle n'est pas encore configuré dans les fichiers du bot.", false);
                }
                ...
                if (existingRole && existingRole.id !== roleId) {
                    return await askConfirmation("div", existingRole.name, roleId);
                }
                ...
                await member.roles.add(roleId).catch(err => 
                    console.error(`[ROLE SYSTEM LOG] Impossible d'ajouter la division (${roleId}) à ${member.user.tag} :`, err.message)
                );

                console.log(`[ROLE SYSTEM LOG] Division mise à jour pour ${member.user.tag} : ${selectedValue}`);
                return await sendResponseEmbed("Profil Mis à Jour", "Votre division compétitive a été enregistrée.", true);
            }

        } catch (error) {
            console.error("[ROLE SYSTEM LOG] Erreur lors du traitement de l'interaction de rôles :", error);
        }
    });
};